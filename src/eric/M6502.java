...
public class M6502 {
  ...
  public void setNZ(int v) {
    if (v == 0) { flags |= zFlag; } else { flags &= nzFlag; }
    if ((v & 0x80) != 0) { flags |= nFlag; } else { flags &= nnFlag; }
  }

  public void setC(boolean v) {
    if (v) { flags |= cFlag; } else { flags &= ncFlag; }
  }

  private void setV(boolean v) {
    if (v) { flags |= vFlag; } else { flags &= nvFlag; }
  }

  private void setI(boolean v) {
    if (v) { flags |= iFlag; } else { flags &= niFlag; }
  }

  private void setD(boolean v) {
    if (v) { flags |= dFlag; } else { flags &= ndFlag; }
  }

  private void setB(boolean v) {
    if (v) { flags |= bFlag; } else { flags &= nbFlag; }
  }

  private void setN(boolean v) {
    if (v) { flags |= nFlag; } else { flags &= nnFlag; }
  }

  private void setZ(boolean v) {
    if (v) { flags |= zFlag; } else { flags &= nzFlag; }
  }

  private void pushByte(int what) {
    mem.write(sp + 256, what); sp--; sp &= 0xff;
  }
  ...
  private int popByte() {
    sp++; sp &= 0xff; return mem.read(sp + 256);
  }
  ...
  private void doASL(int mode) {
    if (mode == mAccum) {
      a <<= 1;
      if (a > 255) { setC(true); a &= 255; } else { setC(false); }
      setNZ(a);
    } else {
      int w = ea(mode);
      int v = mem.read(w);
      v <<= 1;
      if (v > 255) { setC(true); v &= 255; } else { setC(false); }
      mem.write(w, v);
      setNZ(v);
    }
  }
  ...
  private void doROL(int mode) {
    if (mode == mAccum) {
      a <<= 1;
      a |= flags & cFlag;
      if (a > 255) { a &= 0xff; setC(true); } else { setC(false); }
      setNZ(a);
    } else {
      int m = ea(mode);
      int v = mem.read(m);
      v <<= 1;
      v |= flags & cFlag;
      if (v > 255) { v &= 0xff; setC(true); } else { setC(false); }
      mem.write(m, v);
      setNZ(v);
    }
  }
  ...
  private void dumpFlags(StringBuffer b) {
    if ((flags & nFlag) == 0) { b.append(" n"); } else { b.append(" N"); }
    if ((flags & vFlag) == 0) { b.append('v'); } else { b.append('V'); }
    if ((flags & rFlag) == 0) { b.append('r'); } else { b.append('R'); }
    if ((flags & bFlag) == 0) { b.append('b'); } else { b.append('B'); }
    if ((flags & dFlag) == 0) { b.append('d'); } else { b.append('D'); }
    if ((flags & iFlag) == 0) { b.append('i'); } else { b.append('I'); }
    if ((flags & zFlag) == 0) { b.append('z'); } else { b.append('Z'); }
    if ((flags & cFlag) == 0) { b.append('c'); } else { b.append('C'); }
    b.append(' ');
  }
  ...
  private static byte cycleCounts[] = {
    7,6,2,1,5,3,5,1,3,2,2,1,6,4,6,4,
    3,5,5,1,5,4,6,1,2,4,2,1,6,4,6,1,
    6,6,2,1,3,3,5,1,4,2,2,1,4,2,6,4,
    3,5,5,1,4,4,6,1,2,4,2,1,4,4,6,1,
    6,6,2,1,3,3,5,1,3,2,2,1,3,4,6,1,
    3,5,5,1,4,4,6,1,2,4,3,1,8,4,6,1,
    6,6,2,1,3,3,5,1,4,2,2,1,6,4,6,1,
    3,5,5,1,4,4,6,1,2,4,4,1,6,4,6,1,
    3,6,2,1,3,3,3,1,2,2,2,1,4,4,4,1,
    3,6,5,1,4,4,4,1,2,5,2,1,4,5,5,1,
    2,6,2,1,3,3,3,1,2,2,2,1,4,4,4,1,
    3,5,5,1,4,4,4,1,2,4,2,1,4,4,4,1,
    2,6,2,1,3,3,5,1,2,2,2,1,4,4,5,1,
    3,5,5,1,4,4,6,1,2,4,3,1,4,4,6,1,
    2,6,2,1,3,3,5,1,2,2,2,1,4,4,6,1,
    3,5,5,1,4,4,6,1,2,4,4,1,4,4,6,1
  };
}
